...
export default function Dashboard() {
  ...
  const handleContinueLearning = () => {
    ...
    const nextProblem = problems.find(p => !progress?.solvedProblems.includes(p.id)) || problems[0];
    if (nextProblem) {
          ...
          <section className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {filteredProblems.map((problem) => {
                const isLocked = problem.unlockedBy && !progress?.solvedProblems.includes(problem.unlockedBy);
